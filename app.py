...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        exp = (datetime.timedelta(days=30) + datetime.datetime.now()).strftime('%Y-%m-%d')
        ...
        try:
            ...
            if f and f.filename != '':
                ...
                if os.path.exists(fpath): 
                    os.remove(fpath)
                if err == 0:
                    res = f'<div class="rc"><h3>✓ المخطط سليم 100%!</h3><p style="color:#15803d;">جاهز للقص، يمكنك تحميله مجاناً كهدية.</p><a href="/download/{f_name}" class="btn-d" style="display:block; text-decoration:none; line-height:45px; background:#2563eb; color:#fff; border-radius:10px;">تحميل مجاناً 📥</a></div>'
                else:
                    res = f'<div class="rc" style="border-color:#fca5a5;background:#fffaf0;"><div class="success-icon" style="background:#f59e0b;">!</div><h3>تم الإصلاح التلقائي بنجاح!</h3><p>الخطوط المصلحة: <strong style="color:#e11d48;font-size:22px;">{err}</strong></p><button onclick="openPaymentModal()" class="btn-d" style="width:100%; padding:14px; background:#10b981; color:white; border:none; border-radius:10px; font-weight:bold; cursor:pointer;">تحميل ملف DXF السليم الآن 🚀</button></div>'
        except Exception as e: 
            res = f'<div class="ec">⚠️ خطأ: {str(e)}</div>'
