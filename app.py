...
app = Flask(__name__)
# إضافة مفتاح سري لتشغيل الجلسات الآمنة وتذكر ملفات الزبائن
...
@app.route('/download')
def download():
    # استدعاء اسم الملف بدقة من جلسة المستخدم المؤمّنة لمنع خطأ Not found للأبد
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            if f and f.filename != '':
                ...
                # تخزين اسم الملف المصلح في الجلسة الآمنة لتذكره عند كتابة الكود
                session['fixed_file'] = fixed_name
                
                if err == 0: res = f'<div class="rc"><h3>✓ المخطط سليم 100%!</h3><p style="color:#15803d;">جاهز للقص، يمكنك تحميله مجاناً.</p><a href="/download" class="btn-d" style="display:block;text-decoration:none;line-height:45px;background:#2563eb;color:#fff;border-radius:10px;">تحميل مجاناً 📥</a></div>'
    ...
    html = html.replace('USE_RESULT', res).replace('USE_DOWNLOAD_URL', '/download').replace('USE_JS_PTS', str(pts))
