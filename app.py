...
def index():
            ...
            if f and f.filename != '':
                ...
                for e in msp.query('LWPOLYLINE LINE'):
                    if e.dxftype() == 'LWPOLYLINE' and not e.closed:
                        ...
                        if len(e) > 0:
                            p = e.get_points()
                            pts.append({"x": float(p), "y": float(p)})
                    elif e.dxftype() == 'LINE':
                        err += 1
                ...
                if is_ar:
                    res = f'''<div class="rc"><h3>✓ تم الإصلاح بنجاح!</h3><p>الأخطاء: <strong>{err}</strong></p><button type="button" onclick="openPaymentModal()" class="btn" style="background:#10b981;">تحميل ملف DXF السليم 🚀</button></div>'''
                else:
    ...
    p_text = "تكتشف منصتنا الأخطاء، وتسد الخطوط المفتوحة، وتجهز ملفك للقص الفوري وبأعلى دقة." if is_ar else "Our tool detects errors, closes open loops, and preps your file for flawless cutting."
    lbl = "اسحب ملف DXF هنا أو اضغط للتصفح" if is_ar else "Drag & Drop DXF file here or Click to browse"
    btn_lbl = "ابدأ الفحص والإصلاح التلقائي الآن" if is_ar else "Start Auto-Scan & Repair Now"
    ...
    <div class="features">
        <div class="f-card"><h4>⚡ إصلاح تلقائي فوري / Instant Auto-Repair</h4><p>الخوارزمية تكتشف الخطوط المفتوحة وتقوم بإغلاقها ومعالجتها أوتوماتيكياً في ثوانٍ. / Automatically detects open loops.</p></div>
        <div class="f-card"><h4>🎯 تحديد دقيق للأخطاء / Precise Error Tracking</h4><p>يتم رسم دوائر حمراء منبّهة حول أماكن المشاكل والقطع غير المتصلة لتسهيل فحصها البصري. / Red alert circles are highlighted.</p></div>
    </div>
    <div class="ft">جميع الحقوق محفوظة © 2026 منصة DXF Fixer - الأردن. / All Rights Reserved.</div>
    ...
    <script>
        ...
        function vAD(){{var v=document.getElementById('bI').value.trim();if(v===""){{alert("Verification required / تفاصيل التأكيد مطلوبة");}}else{{window.location.href="{d_url}";closePaymentModal();}}}}
